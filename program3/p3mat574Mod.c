...
void sortFlights(Flight flightM[], int length) // from the bubble sort code
{
    int i, j;
    char temp[MAX_FLIGHT_ID_SIZE];
	...
	for (i = 0; i < (length – 1); i++)
	{
		for (j = 0; j < (length – i – 1); j++)
		{
			...
			{
				//exchange [j+1] with [j]
				temp = flightM[j+1].szFlightId;
				flightM[j+1].szFlightId = flightM[j].szFlightId;
				flightM[j].szFlightId = temp;
			}
